...
public class RCUController {
    ...
    ResponseEntity<RCUCaseResponseDTO> AssignedRCUCase(@PathVariable String rcuCaseId, @PathVariable String assignedUser){
    RCUCaseResponseDTO rcuCaseResponseDTO = rcuService.AssignedRCUCase(UUID.fromString(rcuCaseId),UUID.fromString(assignedUser));
    return ResponseEntity.status(HttpStatus.OK).body(rcuCaseResponseDTO);
    }
